...
import { getTopHeroesGlobalByRarity } from "@/application/use-cases/getTopHeroesGlobalByRarity";
...
export default async function RankingStakeHero({
  ...
  let data;
  try {
    data = await getTopHeroesGlobalByRarity({
      network,
      token,
  ...
  const columnPosition =
    token === "bcoin" ? "positionBcoinGlobal" : "positionSenGlobal";
  const columnStake = token === "bcoin" ? "stake" : "stakeSen";

  function getFooterComponent(hero: HeroEntity) {
    const icon =
      hero.network === WalletNetwork.POLYGON
        ? "/images/matic.webp"
        : "/images/bnb.webp";
    return (
      <div className="px-4 pb-2 text-14 justify-center flex flex-row items-center gap-2 md:justify-start ">
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4">
        {data?.heroes?.map((hero) => (
          <Hero
            key={hero.heroId}
            data={hero.hero as HeroEntity}
            positionRanking={hero[columnPosition]}
            footer={getFooterComponent(hero.hero as HeroEntity)}
          />
        ))}
      </div>
    </div>
